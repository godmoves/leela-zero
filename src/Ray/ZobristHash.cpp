...
unsigned int TransHash(const unsigned long long hash) {
...
void ClearNotDescendentNodes(vector<int> &indexes) {
...
unsigned int FindSameHashIndex(const unsigned long long hash, const int color,
  ...
  do {
    if (!node_hash[i].flag) {
      return uct_hash_size;
    } else if (node_hash[i].hash == hash && node_hash[i].color == color &&
               node_hash[i].moves == moves) {
      return i;
    }
    i++;
...
bool CheckRemainingHashSize(void) { return enough_size; }
