...
static void AddStoneToString(search_game_info_t *game, string_t *string,
                             const int pos);

static void AddStone(search_game_info_t *game, const int pos, const int color,
                     const int id);

static void ConnectString(search_game_info_t *game, const int pos,
                          const int color, const int connection,
                          const int id[]);

static bool IsSuicide(const search_game_info_t *game, const string_t *string,
                      const int color, const int pos);

static void MakeString(search_game_info_t *game, const int pos,
                       const int color);

static void MergeLiberty(string_t *dst, string_t *src);

static void MergeNeighbor(string_t *string, string_t *dst, string_t *src,
                          const int id, const int rm_id);

static void MergeStones(search_game_info_t *game, const int id,
                        const int rm_id);

static void MergeString(search_game_info_t *game, string_t *dst,
                        string_t *src[3], const int n);

static void RecordString(search_game_info_t *game, int id);

static void RemoveLiberty(search_game_info_t *game, string_t *string,
                          const int pos);
...
static void RestoreChain(search_game_info_t *game, const int id,
                         const int stone[], const int stones, const int color);
...
bool IsLegalForSearch(const search_game_info_t *game, const int pos,
                      const int color) {
...
void PutStoneForSearch(search_game_info_t *game, const int pos,
                       const int color) {
...
void Undo(search_game_info_t *game) {
  ...
  for (int i = 0; i < rec->strings; i++) {
    ...
    RestoreChain(game, rec->strings_id[i], rec->stone[i], rec->stones[i],
                 rec->string_color[i]);
    rec->stones[i] = 0;
  }
