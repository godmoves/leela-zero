...
static const unsigned int update_mask[40][3] = {
    ...
    {0, 0x00004000, 0x00008000}, {0, 0x00001000, 0x00002000},
    {0, 0x00000400, 0x00000800}, {0, 0x00000100, 0x00000200},
    {0, 0x00000040, 0x00000080}, {0, 0x00000010, 0x00000020},
    {0, 0x00000004, 0x00000008}, {0, 0x00000001, 0x00000002},

    {0, 0x00100000, 0x00200000}, {0, 0x00400000, 0x00800000},
    {0, 0x00010000, 0x00020000}, {0, 0x00040000, 0x00080000},

    {0, 0x00001000, 0x00002000}, {0, 0x00004000, 0x00008000},
    {0, 0x00010000, 0x00020000}, {0, 0x00040000, 0x00080000},
    {0, 0x00100000, 0x00200000}, {0, 0x00400000, 0x00800000},
    {0, 0x00000001, 0x00000002}, {0, 0x00000004, 0x00000008},
    {0, 0x00000010, 0x00000020}, {0, 0x00000040, 0x00000080},
    {0, 0x00000100, 0x00000200}, {0, 0x00000400, 0x00000800},

    {0, 0x00010000, 0x00020000}, {0, 0x00040000, 0x00080000},
    {0, 0x00100000, 0x00200000}, {0, 0x00400000, 0x00800000},
    {0, 0x01000000, 0x02000000}, {0, 0x04000000, 0x08000000},
    {0, 0x10000000, 0x20000000}, {0, 0x40000000, 0x80000000},
    {0, 0x00000001, 0x00000002}, {0, 0x00000004, 0x00000008},
    {0, 0x00000010, 0x00000020}, {0, 0x00000040, 0x00000080},
    {0, 0x00000100, 0x00000200}, {0, 0x00000400, 0x00000800},
    {0, 0x00001000, 0x00002000}, {0, 0x00004000, 0x00008000},
...
void ClearPattern(pattern_t *pat) {
  ...
  for (y = board_start; y <= board_end; y++) {
    ...
    pat[POS(y, board_start)].list[MD_2] |= 0x0003003F;
    pat[POS(y, board_start)].list[MD_3] |= 0x00F0003F;
    pat[POS(y, board_start)].list[MD_4] |= 0xFC0000FF;
    pat[POS(y, board_start)].large_list[MD_5] |= 0xFF000003FF;

    pat[POS(board_end, y)].list[MD_2] |= 0x000CC330;
    pat[POS(board_end, y)].list[MD_3] |= 0x00000FFC;
    pat[POS(board_end, y)].list[MD_4] |= 0x0000FFFC;
    pat[POS(board_end, y)].large_list[MD_5] |= 0x00000FFFFC;

    pat[POS(y, board_end)].list[MD_2] |= 0x0030FC00;
    pat[POS(y, board_end)].list[MD_3] |= 0x0003FF00;
    pat[POS(y, board_end)].list[MD_4] |= 0x00FFFC00;
    pat[POS(y, board_end)].large_list[MD_5] |= 0x003FFFF000;

    pat[POS(board_start, y)].list[MD_2] |= 0x00C00CC3;
    pat[POS(board_start, y)].list[MD_3] |= 0x00FFC000;
    pat[POS(board_start, y)].list[MD_4] |= 0xFFFC0000;
    pat[POS(board_start, y)].large_list[MD_5] |= 0xFFFFC00000;

    pat[POS(y, board_start + 1)].list[MD_2] |= 0x00030000;
    pat[POS(y, board_start + 1)].list[MD_3] |= 0x00C0000F;
    pat[POS(y, board_start + 1)].list[MD_4] |= 0xF000003F;
    pat[POS(y, board_start + 1)].large_list[MD_5] |= 0xFC000000FF;

    pat[POS(board_end - 1, y)].list[MD_2] |= 0x000C0000;
    pat[POS(board_end - 1, y)].list[MD_3] |= 0x000003F0;
    pat[POS(board_end - 1, y)].list[MD_4] |= 0x00003FF0;
    pat[POS(board_end - 1, y)].large_list[MD_5] |= 0x000003FFF0;

    pat[POS(y, board_end - 1)].list[MD_2] |= 0x00300000;
    pat[POS(y, board_end - 1)].list[MD_3] |= 0x0000FC00;
    pat[POS(y, board_end - 1)].list[MD_4] |= 0x003FF000;
    pat[POS(y, board_end - 1)].large_list[MD_5] |= 0x000FFFC000;

    pat[POS(board_start + 1, y)].list[MD_2] |= 0x00C00000;
    pat[POS(board_start + 1, y)].list[MD_3] |= 0x003F0000;
    pat[POS(board_start + 1, y)].list[MD_4] |= 0x3FF00000;
    pat[POS(board_start + 1, y)].large_list[MD_5] |= 0x3FFF000000;

    pat[POS(y, board_start + 2)].list[MD_3] |= 0x00000003;
    pat[POS(y, board_start + 2)].list[MD_4] |= 0xC000000F;
    pat[POS(y, board_start + 2)].large_list[MD_5] |= 0xF00000003F;

    pat[POS(board_end - 2, y)].list[MD_3] |= 0x000000C0;
    pat[POS(board_end - 2, y)].list[MD_4] |= 0x00000FC0;
    pat[POS(board_end - 2, y)].large_list[MD_5] |= 0x000000FFC0;

    pat[POS(y, board_end - 2)].list[MD_3] |= 0x00003000;
    pat[POS(y, board_end - 2)].list[MD_4] |= 0x000FC000;
    pat[POS(y, board_end - 2)].large_list[MD_5] |= 0x0003FF0000;

    pat[POS(board_start + 2, y)].list[MD_3] |= 0x000C0000;
    pat[POS(board_start + 2, y)].list[MD_4] |= 0x0FC00000;
    pat[POS(board_start + 2, y)].large_list[MD_5] |= 0x0FFC000000;

    pat[POS(y, board_start + 3)].list[MD_4] |= 0x00000003;
    pat[POS(y, board_start + 3)].large_list[MD_5] |= 0xC00000000F;

    pat[POS(board_end - 3, y)].list[MD_4] |= 0x00000300;
    pat[POS(board_end - 3, y)].large_list[MD_5] |= 0x0000003F00;

    pat[POS(y, board_end - 3)].list[MD_4] |= 0x00030000;
    pat[POS(y, board_end - 3)].large_list[MD_5] |= 0x0000FC0000;

    pat[POS(board_start + 3, y)].list[MD_4] |= 0x03000000;
...
unsigned int MD2VerticalMirror(const unsigned int md2) {
  return ((md2 & 0x00FC00) >> 10) | (md2 & 0x0003C0) |
         ((md2 & 0x00003F) << 10) | (REV2((md2 & 0x330000) >> 16) << 16) |
         (md2 & 0xCC0000);
}

unsigned int MD3VerticalMirror(const unsigned int md3) {
  return (REV6(md3 & 0x003003)) | (REV4((md3 & 0x000C0C) >> 2) << 2) |
         (REV2((md3 & 0x000330) >> 4) << 4) |
         (REV4((md3 & 0xC0C000) >> 14) << 14) |
         (REV2((md3 & 0x330000) >> 16) << 16) | (md3 & 0x0C00C0);
}

unsigned int MD4VerticalMirror(const unsigned int md4) {
  return (REV8(md4 & 0x00030003)) | (REV6((md4 & 0x0000C00C) >> 2) << 2) |
         (REV4((md4 & 0x00003030) >> 4) << 4) |
         (REV2((md4 & 0x00000CC0) >> 6) << 6) |
         (REV6((md4 & 0xC00C0000) >> 18) << 18) |
         (REV4((md4 & 0x30300000) >> 20) << 20) |
         (REV2((md4 & 0x0CC00000) >> 22) << 22) | (md4 & 0x03000300);
}

unsigned long long MD5VerticalMirror(const unsigned long long md5) {
  return (REV10(md5 & 0x0000300003)) | (REV8((md5 & 0x00000C000C) >> 2) << 2) |
         ...
         (REV2((md5 & 0x0330000000) >> 28) << 28) | (md5 & 0x00C0000C00);
}

unsigned int Pat3HorizontalMirror(const unsigned int pat3) {
  return (REV3((pat3 & 0xFC00) >> 10) << 10) |
         (REV((pat3 & 0x03C0) >> 6) << 6) | REV3((pat3 & 0x003F));
}

unsigned int MD2HorizontalMirror(const unsigned int md2) {
  return (REV3((md2 & 0x00FC00) >> 10) << 10) |
         (REV((md2 & 0x0003C0) >> 6) << 6) | REV3((md2 & 0x00003F)) |
         (REV2((md2 & 0xCC0000) >> 18) << 18) | (md2 & 0x330000);
}

unsigned int MD3HorizontalMirror(const unsigned int md3) {
  return (md3 & 0x003003) | (REV10((md3 & 0xC0000C) >> 2) << 2) |
         (REV8((md3 & 0x300030) >> 4) << 4) |
         (REV6((md3 & 0x0C00C0) >> 6) << 6) |
         (REV4((md3 & 0x030300) >> 8) << 8) |
         (REV2((md3 & 0x00CC00) >> 10) << 10);
}

unsigned int MD4HorizontalMirror(const unsigned int md4) {
  return (md4 & 0x00030003) | (REV14((md4 & 0xC000000C) >> 2) << 2) |
         (REV12((md4 & 0x30000030) >> 4) << 4) |
         (REV10((md4 & 0x0C0000C0) >> 6) << 6) |
         (REV8((md4 & 0x03000300) >> 8) << 8) |
         (REV6((md4 & 0x00C00C00) >> 10) << 10) |
         (REV4((md4 & 0x00303000) >> 12) << 12) |
         (REV2((md4 & 0x000CC000) >> 14) << 14);
}

unsigned long long MD5HorizontalMirror(const unsigned long long md5) {
  return (md5 & 0x0000300003) | (REV18((md5 & 0xC00000000C) >> 2) << 2) |
...
unsigned int Pat3Rotate90(const unsigned int pat3) {
  ...
  return ((pat3 & 0x0003) << 10) | ((pat3 & 0x0C0C) << 4) |
         ((pat3 & 0x3030) >> 4) | ((pat3 & 0x00C0) << 6) |
         ((pat3 & 0x0300) >> 6) | ((pat3 & 0xC000) >> 10);
}

unsigned int MD2Rotate90(const unsigned int md2) {
  return ((md2 & 0x000003) << 10) | ((md2 & 0x000C0C) << 4) |
         ((md2 & 0x003030) >> 4) | ((md2 & 0x0300C0) << 6) |
         ((md2 & 0x000300) >> 6) | ((md2 & 0x00C000) >> 10) |
         ((md2 & 0xFC0000) >> 2);
}

unsigned int MD3Rotate90(const unsigned int md3) {
  return ((md3 & 0x00003F) << 18) | ((md3 & 0xFFFFC0) >> 6);
}

unsigned int MD4Rotate90(const unsigned int md4) {
  return ((md4 & 0x000000FF) << 24) | ((md4 & 0xFFFFFF00) >> 8);
}

unsigned long long MD5Rotate90(const unsigned long long md5) {
  return ((md5 & 0x00000003FF) << 30) | ((md5 & 0xFFFFFFFC00) >> 10);
}
...
void DisplayInputPat3(const unsigned int pat3) {
  ...
  printf("\n");
  printf("%c%c%c\n", stone[pat3 & 0x3], stone[(pat3 >> 2) & 0x3],
         stone[(pat3 >> 4) & 0x3]);
  printf("%c*%c\n", stone[(pat3 >> 6) & 0x3], stone[(pat3 >> 8) & 0x3]);
  printf("%c%c%c\n", stone[(pat3 >> 10) & 0x3], stone[(pat3 >> 12) & 0x3],
         stone[(pat3 >> 14) & 0x3]);
}
...
void DisplayInputMD2(const unsigned int md2) {
  ...
  printf("\n");
  printf("  %c  \n", stone[(md2 >> 16) & 0x3]);
  printf(" %c%c%c \n", stone[md2 & 0x3], stone[(md2 >> 2) & 0x3],
         stone[(md2 >> 4) & 0x3]);
  printf("%c%c*%c%c\n", stone[(md2 >> 22) & 0x3], stone[(md2 >> 6) & 0x3],
         stone[(md2 >> 8) & 0x3], stone[(md2 >> 18) & 0x3]);
  printf(" %c%c%c \n", stone[(md2 >> 10) & 0x3], stone[(md2 >> 12) & 0x3],
         stone[(md2 >> 14) & 0x3]);
  printf("  %c  \n", stone[(md2 >> 20) & 0x3]);
}
...
void DisplayInputPattern(pattern_t *pattern, int size) {
  static char stone[] = {'+', '@', 'O', '#'};
  ...
  if (size == 4) {
    printf("\n");
    printf("    %c    \n", stone[md4 & 0x3]);
    printf("   %c%c%c   \n", stone[(md4 >> 30) & 0x3], stone[md3 & 0x3],
           stone[(md4 >> 2) & 0x3]);
    printf("  %c%c%c%c%c  \n", stone[(md4 >> 28) & 0x3],
           stone[(md3 >> 22) & 0x3], stone[(md2 >> 16) & 0x3],
           stone[(md3 >> 2) & 0x3], stone[(md4 >> 4) & 0x3]);
    printf(" %c%c%c%c%c%c%c \n", stone[(md4 >> 26) & 0x3],
           stone[(md3 >> 20) & 0x3], stone[md2 & 0x3], stone[(md2 >> 2) & 0x3],
           stone[(md2 >> 4) & 0x3], stone[(md3 >> 4) & 0x3],
           stone[(md4 >> 6) & 0x3]);
    printf("%c%c%c%c*%c%c%c%c\n", stone[(md4 >> 24) & 0x3],
           stone[(md3 >> 18) & 0x3], stone[(md2 >> 22) & 0x3],
           stone[(md2 >> 6) & 0x3], stone[(md2 >> 8) & 0x3],
           stone[(md2 >> 18) & 0x3], stone[(md3 >> 6) & 0x3],
           stone[(md4 >> 8) & 0x3]);
    printf(" %c%c%c%c%c%c%c \n", stone[(md4 >> 22) & 0x3],
           stone[(md3 >> 16) & 0x3], stone[(md2 >> 10) & 0x3],
           stone[(md2 >> 12) & 0x3], stone[(md2 >> 14) & 0x3],
           stone[(md3 >> 8) & 0x3], stone[(md4 >> 10) & 0x3]);
    printf("  %c%c%c%c%c  \n", stone[(md4 >> 20) & 0x3],
           stone[(md3 >> 14) & 0x3], stone[(md2 >> 20) & 0x3],
           stone[(md3 >> 10) & 0x3], stone[(md4 >> 12) & 0x3]);
    printf("   %c%c%c   \n", stone[(md4 >> 18) & 0x3], stone[(md3 >> 12) & 0x3],
           stone[(md4 >> 14) & 0x3]);
    printf("    %c    \n", stone[(md4 >> 16) & 0x3]);
  } else if (size == 3) {
    printf("\n");
    ...
    printf("  %c%c%c  \n", stone[(md3 >> 22) & 0x3], stone[(md2 >> 16) & 0x3],
           stone[(md3 >> 2) & 0x3]);
    printf(" %c%c%c%c%c \n", stone[(md3 >> 20) & 0x3], stone[md2 & 0x3],
           stone[(md2 >> 2) & 0x3], stone[(md2 >> 4) & 0x3],
           stone[(md3 >> 4) & 0x3]);
    printf("%c%c%c*%c%c%c\n", stone[(md3 >> 18) & 0x3],
           stone[(md2 >> 22) & 0x3], stone[(md2 >> 6) & 0x3],
           stone[(md2 >> 8) & 0x3], stone[(md2 >> 18) & 0x3],
