...
static void CalculateNextPlayouts(game_info_t *game, int color, double best_wp,
                                  double finish_time);

static void CalculateCriticality(int color);

static void CalculateCriticalityIndex(uct_node_t *node,
                                      statistic_t *node_statistic, int color,
                                      int *index);

static void CalculateOwner(int color, int count);

static void CalculateOwnerIndex(uct_node_t *node, statistic_t *node_statistc,
                                int color, int *index);
...
static int UctSearch(game_info_t *game, int color, std::mt19937_64 *mt,
                     int current, int *winner);

static void UpdateNodeStatistic(game_info_t *game, int winner,
                                statistic_t *node_statistic);

static void UpdateResult(child_node_t *child, int result, int current);

void SetPonderingMode(bool flag) { pondering_mode = flag; }

void SetMode(enum SEARCH_MODE new_mode) { mode = new_mode; }

void SetPlayout(int po) { playout = po; }

void SetConstTime(double time) { const_thinking_time = time; }

void SetThread(int new_thread) { threads = new_thread; }

void SetTime(double time) { default_remaining_time = time; }

void SetReuseSubtree(bool flag) { reuse_subtree = flag; }
...
void SetTimeSettings(int main_time, int byoyomi, int stone) {
  if (mode == CONST_PLAYOUT_MODE || mode == CONST_TIME_MODE) {
    return;
  }
...
int UctSearchGenmove(game_info_t *game, int color) {
  ...
  if (game->moves > pure_board_size * 3 - 17 && extend_time && ExtendTime()) {
  ...
  if (uct_child[PASS_INDEX].move_count != 0) {
    pass_wp =
        (double)uct_child[PASS_INDEX].win / uct_child[PASS_INDEX].move_count;
  } else {
    pass_wp = 0;
  }

  best_wp =
      (double)uct_child[select_index].win / uct_child[select_index].move_count;
  ...
  if (pass_wp >= PASS_THRESHOLD &&
    ...
    pos = PASS;
  } else if (game->moves > 3 && game->record[game->moves - 1].pos == PASS &&
  ...
  PrintPlayoutInformation(&uct_node[current_root], &po_info, finish_time,
                          pre_simulated);
...
static void RatingNode(game_info_t *game, int color, int index) {
  ...
  uct_child[PASS_INDEX].rate =
      CalculateLFRScore(game, PASS, pat_index, &uct_features);
  ...
  for (int i = 1; i < child_num; i++) {
    ...
    dynamic_parameter =
        uct_owner[owner_index[pos]] + uct_criticality[criticality_index[pos]];
...
static int UctSearch(game_info_t *game, int color, mt19937_64 *mt, int current,
                     int *winner) {
...
static int SelectMaxUcbChild(int current, int color) {
  ...
  int pos;
  int width;
  const double ucb_bonus_weight =
      bonus_weight * sqrt(bonus_equivalence / (sum + bonus_equivalence));

  if ((sum & 0x7f) == 0 && sum != 0) {
    int o_index[UCT_CHILD_MAX], c_index[UCT_CHILD_MAX];
    CalculateCriticalityIndex(&uct_node[current], uct_node[current].statistic,
                              color, c_index);
    CalculateOwnerIndex(&uct_node[current], uct_node[current].statistic, color,
                        o_index);
    for (int i = 0; i < child_num; i++) {
      pos = uct_child[i].pos;
      if (pos == PASS) {
    ...
    width = ((uct_node[current].width > child_num) ? child_num
  ...
  if (sum > pw[uct_node[current].width]) {
    ...
    for (int i = 0; i < child_num; i++) {
      if (uct_child[i].flag == false) {
        pos = uct_child[i].pos;
        dynamic_parameter = uct_owner[owner_index[pos]] +
                            uct_criticality[criticality_index[pos]];
...
static void CalculateCriticalityIndex(uct_node_t *node,
  ...
  for (int i = 1; i < child_num; i++) {
    ...
    if (tmp < 0)
      tmp = 0;
    index[i] = (int)(tmp * 40);
    if (index[i] > criticality_max - 1)
      index[i] = criticality_max - 1;
  }
}
...
static void CalculateCriticality(int color) {
  ...
  for (int i = 0; i < pure_board_max; i++) {
    ...
    if (criticality_index[pos] > criticality_max - 1)
      criticality_index[pos] = criticality_max - 1;
  }
}

static void CalculateOwnerIndex(uct_node_t *node, statistic_t *node_statistic,
                                int color, int *index) {
  int pos;
  const int count = node->move_count;
  const int child_num = node->child_num;
  ...
  for (int i = 1; i < child_num; i++) {
    pos = node->child[i].pos;
    index[i] =
        (int)((double)node_statistic[pos].colors[color] * 10.0 / count + 0.5);
    if (index[i] > OWNER_MAX - 1)
      index[i] = OWNER_MAX - 1;
    if (index[i] < 0)
      index[pos] = 0;
  }
...
static void CalculateOwner(int color, int count) {
  ...
  for (int i = 0; i < pure_board_max; i++) {
    pos = onboard_pos[i];
    owner_index[pos] =
        (int)((double)statistic[pos].colors[color] * 10.0 / count + 0.5);
    if (owner_index[pos] > OWNER_MAX - 1)
      owner_index[pos] = OWNER_MAX - 1;
    if (owner_index[pos] < 0)
      owner_index[pos] = 0;
  }
}
...
static void CalculateNextPlayouts(game_info_t *game, int color, double best_wp,
  ...
  if (mode == CONST_TIME_MODE) {
    if (best_wp > 0.90) {
      po_info.num =
          (int)(po_info.count / finish_time * const_thinking_time / 2);
    } else {
    ...
    if (pure_board_size < 11) {
      ...
      time_limit = remaining_time[color] /
                   (TIME_C_13 + ((TIME_MAXPLY_13 - (game->moves + 1) > 0)
                                     ? TIME_MAXPLY_13 - (game->moves + 1)
                                     : 0));
    } else {
      time_limit = remaining_time[color] /
                   (TIME_C_19 + ((TIME_MAXPLY_19 - (game->moves + 1) > 0)
                                     ? TIME_MAXPLY_19 - (game->moves + 1)
                                     : 0));
    }
...
int UctAnalyze(game_info_t *game, int color) {
  ...
  for (y = board_start; y <= board_end; y++) {
    for (x = board_start; x <= board_end; x++) {
      pos = POS(x, y);
      own = (double)statistic[pos].colors[S_BLACK] /
            uct_node[current_root].move_count;
      if (own > 0.5) {
        black++;
      } else {
        white++;
...
void OwnerCopy(int *dest) {
  ...
  for (int i = 0; i < pure_board_max; i++) {
    pos = onboard_pos[i];
    dest[pos] =
        (int)((double)uct_node[current_root].statistic[pos].colors[my_color] /
              uct_node[current_root].move_count * 100);
  }
}
...
int UctSearchGenmoveCleanUp(game_info_t *game, int color) {
  ...
  if ((double)uct_child[select_index].win / uct_child[select_index].move_count <
      RESIGN_THRESHOLD) {
    pos = PASS;
  }
