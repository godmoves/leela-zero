...
static void PoCheckFeaturesLib1(game_info_t *game, const int color,
                                const int id, int *update, int *update_num);

static void PoCheckFeaturesLib2(game_info_t *game, const int color,
                                const int id, int *update, int *update_num);

static void PoCheckFeaturesLib3(game_info_t *game, const int color,
                                const int id, int *update, int *update_num);

static void PoCheckFeatures(game_info_t *game, const int color, int *update,
                            int *update_num);

static void PoCheckCaptureAfterKo(game_info_t *game, const int color,
                                  int *update, int *update_num);

static bool PoCheckSelfAtari(game_info_t *game, const int color, const int pos);

static void PoCheckCaptureAndAtari(game_info_t *game, const int color,
                                   const int pos);

static void PoCheckRemove2Stones(game_info_t *game, const int color,
                                 int *update, int *update_num);
...
void Neighbor12(int previous_move, int distance_2[], int distance_3[],
                int distance_4[]) {
...
void NeighborUpdate(game_info_t *game, int color, long long *sum_rate,
                    long long *sum_rate_row, long long *rate, int *update,
                    bool *flag, int index) {
  int i, pos;
  double gamma;
  double bias[4];
...
void NakadeUpdate(game_info_t *game, int color, long long *sum_rate,
                  long long *sum_rate_row, long long *rate, int *nakade_pos,
                  int nakade_num, bool *flag, int pm1) {
  int i, pos, dis;
  double gamma;
  bool self_atari_flag;
...
void OtherUpdate(game_info_t *game, int color, long long *sum_rate,
                 long long *sum_rate_row, long long *rate, int update_num,
...
void Neighbor12Update(game_info_t *game, int color, long long *sum_rate,
                      long long *sum_rate_row, long long *rate, int update_num,
                      int *update, bool *flag) {
  int i, j, pos;
  double gamma;
...
void PartialRating(game_info_t *game, int color, long long *sum_rate,
                   long long *sum_rate_row, long long *rate) {
  ...
  *update_num = 0;
  ...
  if (game->moves > 2)
    pm2 = game->record[game->moves - 2].pos;
  if (game->moves > 3)
    pm3 = game->record[game->moves - 3].pos;
  ...
  if (pm1 != PASS) {
    ...
    NakadeUpdate(game, color, sum_rate, sum_rate_row, rate, nakade_pos,
                 nakade_num, flag, pm1);

    NeighborUpdate(game, color, sum_rate, sum_rate_row, rate, distance_2, flag,
                   0);

    NeighborUpdate(game, color, sum_rate, sum_rate_row, rate, distance_3, flag,
                   1);

    NeighborUpdate(game, color, sum_rate, sum_rate_row, rate, distance_4, flag,
                   2);
  }

  if (pm2 != PASS)
    Neighbor12Update(game, color, sum_rate, sum_rate_row, rate, 1, &pm2, flag);

  if (pm3 != PASS)
    Neighbor12Update(game, color, sum_rate, sum_rate_row, rate, 1, &pm3, flag);

  OtherUpdate(game, color, sum_rate, sum_rate_row, rate, prev_feature,
              prev_feature_pos, flag);

  OtherUpdate(game, color, sum_rate, sum_rate_row, rate,
              game->update_num[color], game->update_pos[color], flag);

  OtherUpdate(game, color, sum_rate, sum_rate_row, rate,
              game->update_num[other], game->update_pos[other], flag);

  Neighbor12Update(game, color, sum_rate, sum_rate_row, rate,
                   game->capture_num[color], game->capture_pos[color], flag);

  Neighbor12Update(game, color, sum_rate, sum_rate_row, rate,
                   game->capture_num[other], game->capture_pos[other], flag);
}

void Rating(game_info_t *game, int color, long long *sum_rate,
            long long *sum_rate_row, long long *rate) {
  int i, pos;
  int pm1 = PASS;
  double gamma;
...
static void PoCheckFeaturesLib1(game_info_t *game, const int color,
  ...
  if (contact) {
    if (string[id].size == 1) {
      game->tactical_features1[lib] |=
          po_tactical_features_mask[F_SAVE_EXTENSION1];
    } else if (string[id].size == 2) {
      game->tactical_features1[lib] |=
          po_tactical_features_mask[F_SAVE_EXTENSION2];
    } else {
      game->tactical_features1[lib] |=
          po_tactical_features_mask[F_SAVE_EXTENSION3];
    }
  } else {
    if (string[id].size == 1) {
      game->tactical_features1[lib] |=
          po_tactical_features_mask[F_SAVE_EXTENSION_SAFELY1];
    } else if (string[id].size == 2) {
      game->tactical_features1[lib] |=
          po_tactical_features_mask[F_SAVE_EXTENSION_SAFELY2];
    } else {
      game->tactical_features1[lib] |=
          po_tactical_features_mask[F_SAVE_EXTENSION_SAFELY3];
    }
  }
    ...
    while (neighbor != NEIGHBOR_END) {
      if (string[neighbor].libs == 1) {
        ...
        if (string[neighbor].size == 1) {
          game->tactical_features1[lib] |=
              po_tactical_features_mask[F_SAVE_CAPTURE1_1];
        } else if (string[neighbor].size == 2) {
          game->tactical_features1[lib] |=
              po_tactical_features_mask[F_SAVE_CAPTURE1_2];
        } else {
          game->tactical_features1[lib] |=
              po_tactical_features_mask[F_SAVE_CAPTURE1_3];
        }
        update[(*update_num)++] = lib;
      }
      ...
      if (string[neighbor].libs == 1) {
        ...
        if (string[neighbor].size == 1) {
          if (IsSelfAtariCaptureForSimulation(game, lib, color, liberty)) {
            game->tactical_features1[lib] |=
                po_tactical_features_mask[F_SAVE_CAPTURE_SELF_ATARI];
          } else {
            game->tactical_features1[lib] |=
                po_tactical_features_mask[F_SAVE_CAPTURE2_1];
          }
        } else if (string[neighbor].size == 2) {
          game->tactical_features1[lib] |=
              po_tactical_features_mask[F_SAVE_CAPTURE2_2];
        } else {
          game->tactical_features1[lib] |=
              po_tactical_features_mask[F_SAVE_CAPTURE2_3];
        }
        update[(*update_num)++] = lib;
      }
      neighbor = string[id].neighbor[neighbor];
      ...
      if (string[neighbor].libs == 1) {
        ...
        if (string[neighbor].size == 1) {
          if (IsSelfAtariCaptureForSimulation(game, lib, color, liberty)) {
            ...
            game->tactical_features1[lib] |=
                po_tactical_features_mask[F_SAVE_CAPTURE3_1];
          }
        } else if (string[neighbor].size == 2) {
          game->tactical_features1[lib] |=
              po_tactical_features_mask[F_SAVE_CAPTURE3_2];
        } else {
          game->tactical_features1[lib] |=
              po_tactical_features_mask[F_SAVE_CAPTURE3_3];
        }
        update[(*update_num)++] = lib;
      }
      neighbor = string[id].neighbor[neighbor];
...
static void PoCheckFeaturesLib2(game_info_t *game, const int color,
  ...
  if (nb4_empty[Pat3(game->pat, lib1)] == 3 ||
    ...
    game->tactical_features2[lib1] |=
        po_tactical_features_mask[F_2POINT_EXTENSION_SAFELY];
  } else {
    game->tactical_features2[lib1] |=
        po_tactical_features_mask[F_2POINT_EXTENSION];
  }
  ...
  while (neighbor != NEIGHBOR_END) {
    if (string[neighbor].libs == 1) {
      ...
      if (string[neighbor].size <= 2) {
        game->tactical_features1[lib1] |=
            po_tactical_features_mask[F_2POINT_CAPTURE_SMALL];
      } else {
        game->tactical_features1[lib1] |=
            po_tactical_features_mask[F_2POINT_CAPTURE_LARGE];
      }
      ...
      if (string[neighbor].size <= 2) {
        if (capturable1) {
          game->tactical_features2[lib1] |=
              po_tactical_features_mask[F_2POINT_C_ATARI_SMALL];
        } else {
          game->tactical_features2[lib1] |=
              po_tactical_features_mask[F_2POINT_ATARI_SMALL];
        }
        if (capturable2) {
          game->tactical_features2[lib2] |=
              po_tactical_features_mask[F_2POINT_C_ATARI_SMALL];
        ...
        if (capturable1) {
          game->tactical_features2[lib1] |=
              po_tactical_features_mask[F_2POINT_C_ATARI_LARGE];
        } else {
          game->tactical_features2[lib1] |=
              po_tactical_features_mask[F_2POINT_ATARI_LARGE];
        }
        if (capturable2) {
          game->tactical_features2[lib2] |=
              po_tactical_features_mask[F_2POINT_C_ATARI_LARGE];
...
static void PoCheckFeaturesLib3(game_info_t *game, const int color,
  ...
  if (nb4_empty[Pat3(game->pat, lib1)] == 3 ||
      ...
      (board[SOUTH(lib1)] == color && string_id[SOUTH(lib1)] != id &&
       string[string_id[SOUTH(lib1)]].libs >= 3)) {
    game->tactical_features2[lib1] |=
        po_tactical_features_mask[F_3POINT_EXTENSION_SAFELY];
  } else {
    game->tactical_features2[lib1] |=
        po_tactical_features_mask[F_3POINT_EXTENSION];
  }
  ...
  while (neighbor != NEIGHBOR_END) {
    if (string[neighbor].libs == 1) {
      ...
      if (string[neighbor].size <= 2) {
        game->tactical_features1[lib1] |=
            po_tactical_features_mask[F_3POINT_CAPTURE_SMALL];
      } else {
        game->tactical_features1[lib1] |=
            po_tactical_features_mask[F_3POINT_CAPTURE_LARGE];
      }
    } else if (string[neighbor].libs == 2) {
      lib1 = string[neighbor].lib[0];
      update[(*update_num)++] = lib1;
      ...
      if (string[neighbor].size <= 2) {
        if (capturable1) {
          game->tactical_features2[lib1] |=
              po_tactical_features_mask[F_3POINT_C_ATARI_SMALL];
        } else {
          game->tactical_features2[lib1] |=
              po_tactical_features_mask[F_3POINT_ATARI_SMALL];
        }
        if (capturable2) {
          game->tactical_features2[lib2] |=
              po_tactical_features_mask[F_3POINT_C_ATARI_SMALL];
        ...
        if (capturable1) {
          game->tactical_features2[lib1] |=
              po_tactical_features_mask[F_3POINT_C_ATARI_LARGE];
        } else {
          game->tactical_features2[lib1] |=
              po_tactical_features_mask[F_3POINT_ATARI_LARGE];
        }
        if (capturable2) {
          game->tactical_features2[lib2] |=
              po_tactical_features_mask[F_3POINT_C_ATARI_LARGE];
      ...
      if (string[neighbor].size <= 2) {
        game->tactical_features2[lib1] |=
            po_tactical_features_mask[F_3POINT_DAME_SMALL];
        game->tactical_features2[lib2] |=
            po_tactical_features_mask[F_3POINT_DAME_SMALL];
        game->tactical_features2[lib3] |=
            po_tactical_features_mask[F_3POINT_DAME_SMALL];
        ...
        game->tactical_features2[lib1] |=
            po_tactical_features_mask[F_3POINT_DAME_LARGE];
        game->tactical_features2[lib2] |=
            po_tactical_features_mask[F_3POINT_DAME_LARGE];
        game->tactical_features2[lib3] |=
            po_tactical_features_mask[F_3POINT_DAME_LARGE];
...
static void PoCheckCaptureAfterKo(game_info_t *game, const int color,
  ...
  if (board[NORTH(previous_move_2)] == other) {
    ...
    if (string[id].libs == 1) {
      ...
      update[(*update_num)++] = lib;
      game->tactical_features1[lib] |=
          po_tactical_features_mask[F_CAPTURE_AFTER_KO];
    }
    check[checked++] = id;
  }
...
static bool PoCheckSelfAtari(game_info_t *game, const int color,
                             const int pos) {
  char *board = game->board;
  string_t *string = game->string;
  int *string_id = game->string_id;
  ...
  if (size < 2) {
    game->tactical_features2[pos] |=
        po_tactical_features_mask[F_SELF_ATARI_SMALL];
    flag = true;
  } else if (size < 5) {
    if (IsNakadeSelfAtari(game, pos, color)) {
      game->tactical_features2[pos] |=
          po_tactical_features_mask[F_SELF_ATARI_NAKADE];
      flag = true;
    } else {
      game->tactical_features2[pos] |=
          po_tactical_features_mask[F_SELF_ATARI_LARGE];
      flag = false;
    }
  } else {
    game->tactical_features2[pos] |=
