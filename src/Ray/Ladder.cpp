...
static bool IsLadderCaptured(const int depth, search_game_info_t *game,
                             const int ren_xy, const int turn_color);
...
void LadderExtension(game_info_t *game, int color, bool *ladder_pos) {
  ...
  for (int i = 0; i < MAX_STRING; i++) {
    if (!string[i].flag || string[i].color != color) {
      continue;
    }
    ...
    if (!checked[ladder] && string[i].libs == 1) {
      ...
      while (neighbor != NEIGHBOR_END && !flag) {
        if (string[neighbor].libs == 1) {
          if (IsLegal(game, string[neighbor].lib[0], color)) {
            ...
            if (IsLadderCaptured(0, ladder_game, string[i].origin,
                                 FLIP_COLOR(color)) == DEAD) {
              if (string[i].size >= 2) {
      ...
      if (!flag) {
        if (IsLegal(game, ladder, color)) {
          ...
          if (string[i].size >= 2 &&
              IsLadderCaptured(0, ladder_game, ladder, FLIP_COLOR(color)) ==
                  DEAD) {
    ...
    while (neighbor != NEIGHBOR_END) {
      if (string[neighbor].libs == 1) {
        if (IsLegalForSearch(game, string[neighbor].lib[0], escape_color)) {
          ...
          result =
              IsLadderCaptured(depth + 1, game, ren_xy, FLIP_COLOR(turn_color));
          Undo(game);
          if (result == ALIVE) {
            return ALIVE;
          }
